...
import { US_STATES, REGION_ALIASES } from './lib/gazetteer.mjs';

const ROOT = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const CACHE = path.join(ROOT, '.cache');
const OUT = path.join(ROOT, 'public/data/bluebook.json');
const UA = 'GodsEyeUAP/0.1 (+https://github.com/domw99/Gods-Eye-UAPs) data build';
...
function regionCentroid(gz, key) {
...
/** Map a trailing region token ("Florida", "Calif", "N-M", "Japan") to a key. */
function resolveRegionToken(gz, token) {
